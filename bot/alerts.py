...
def format_help_message() -> str:
    return "\n".join([
        ...
        f"  /picks          — Top player prop opportunities by tier (S→A→B)",
        f"                    <i>/picks 5 · /picks MLB · /picks NBA 5</i>",
        f"  /slip [N]       — Build a prop slip from top N picks (2–6)",
        f"                    <i>/slip 3 · /slip 5</i>",
        f"  /dashboard      — Player prop activity, provider status, alert history",
        f"  /alerts         — Recent player prop alert history",
        f"  /grade          — Win/loss breakdown by tier for resolved picks",
        "",
        "<b>📊 Market Comparison</b>",
        f"  /market   — Cross-provider line comparison",
        f"  /clv      — Closing Line Value history",
        "",
        "<b>📈 Performance</b>",
        f"  /performance    — Win rate, CLV and ROI by tier & sport",
        f"  /backtest       — Replay resolved alerts through the model",
        f"  /stats          — Alert generation stats and outcome summary",
        f"  /calibration    — Model calibration: tier accuracy vs outcomes",
        "",
        "<b>📥 Manual Data</b>",
        f"  /pp_import      — Import PrizePicks lines (pipe-delimited, one prop per line)",
        f"                    <i>/pp_import\nFreddy Peralta | Strikeouts | 5.5 | MLB</i>",
        "",
        "<b>⚙️ System</b>",
        ...
        f"  /testalert  — Send a mock Player Prop Market Alert to verify delivery",
        f"  /help       — This menu",
        "",
        "<b>Active Providers</b>",
        "  🟣 PrizePicks  ·  🐶 Underdog  ·  🎰 DraftKings  ·  🦊 FanDuel",
        "",
        f"<i>Monitoring player props across all providers. Not betting advice.</i>",
    ])
...
def format_start_message() -> str:
    return "\n".join([
        ...
        "",
        "I monitor player props across all major providers and surface",
        "<b>meaningful line moves and market opportunities</b> in real time.",
        "",
        "<b>What I do:</b>",
        f"  🟣 Track PrizePicks lines (via manual import)",
        f"  🐶 Monitor Underdog Fantasy live (every 5 min)",
        f"  🎰 Watch DraftKings and 🦊 FanDuel prop lines",
        f"  📊 Compare providers to find the best available line",
        f"  📈 Detect line movement and market shifts automatically",
        f"  🔔 Alert you when qualifying opportunities appear",
        "",
        "<b>Alert format:</b>  🟣 PLAYER PROP MARKET ALERT",
        "  Shows available lines from every provider,",
        "  best line, market consensus, and movement.",
        "",
        "Use /help to see all commands.",
        "Use /pp_import to add PrizePicks data.",
        "",
        f"<i>Monitoring player props. Not betting advice.</i>",
    ])
