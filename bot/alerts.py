...
def format_help_message() -> str:
    return "\n".join([
        ...
        "",
        "<b>🎯 PrizePicks Interface</b>",
        f"  /picks          — Ranked PP edges by tier (S→A→B), last 6 h",
        ...
        "",
        "<b>📊 Market & Analysis</b>",
        f"  /analyze  — Manually analyze a betting line",
        f"              <i>/analyze NFL Chiefs-3 -110 -110</i>",
        f"  /steam    — Latest {EMOJI['fire']} steam / sharp moves",
        f"  /ev       — Latest {EMOJI['ev']} +EV opportunities",
        f"  /market   — Cross-book consensus and inefficiencies",
        f"  /clv      — Closing Line Value history",
        "",
        "<b>📈 Performance & Calibration</b>",
        f"  /performance    — Win rate, CLV and ROI by tier & sport",
        f"  /backtest       — Replay last N resolved alerts through the model",
        f"  /stats          — Alert generation stats and outcome summary",
        f"  /calibration    — Model calibration: tier accuracy, CLV, detection vs recommendation",
        "",
        "<b>📥 Manual Data</b>",
        f"  /pp_import      — Import PrizePicks prop data (pipe-delimited, one prop per line)",
        f"                    <i>/pp_import\nLeBron James | Points | 25.5 | NBA</i>",
        "",
        "<b>⚙️ System</b>",
