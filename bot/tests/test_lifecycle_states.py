...
# ── Event loop fixture (module-scoped, Python 3.11 compatible) ────────────────
...
class TestLifecycleStateDefault:
    def test_default_is_discovered(self, db):
        from database import PropLineHistory
        ...
        async def _run():
            async with db.session() as s:
                ...
                await s.commit()
                await s.refresh(row)
                # Default value applied at insert
    ...
    def test_first_alert_sent_at_null_by_default(self, db):
        from database import PropLineHistory
        from sqlalchemy import select
...
class TestUpdatePropLifecycleState:
    def test_returns_false_when_no_row(self, db):
        updated = run(db.update_prop_lifecycle_state(
            provider    = "Underdog",
            player_name = "Nobody",
            sport       = "MLB",
    ...
    def test_returns_true_and_updates_state(self, db):
        from database import PropLineHistory
        from sqlalchemy import select
    ...
    def test_sets_first_alert_sent_at_when_none(self, db):
        from database import PropLineHistory
        from sqlalchemy import select
        ...
        run(db.update_prop_lifecycle_state(
            provider              = "Underdog",
            player_name           = "Mike Trout",
            sport                 = "MLB",
            stat_type             = "RBIs",
    ...
    def test_does_not_overwrite_existing_first_alert_sent_at(self, db):
        from database import PropLineHistory
    ...
    def test_idempotent_multiple_calls(self, db):
        ...
        for _ in range(3):
            result = run(db.update_prop_lifecycle_state(
                provider    = "Underdog",
                player_name = "Freddie Freeman",
                sport       = "MLB",
                stat_type   = "Hits",
