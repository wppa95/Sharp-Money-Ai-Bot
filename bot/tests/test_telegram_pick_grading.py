...
@pytest.fixture()
async def db():
    """Fresh in-memory database for each test, properly closed on teardown."""
...
class TestAlertSentDefault:
    async def test_evaluated_prop_not_sent_has_alert_sent_false(self, db):
    ...
    async def test_pass_recommendation_not_alert_sent(self, db):
...
class TestMarkAlertSent:
    async def test_mark_sets_alert_sent_true(self, db):
    ...
    async def test_mark_sets_alert_sent_at_timestamp(self, db):
    ...
    async def test_mark_noop_on_nonexistent_row(self, db):
    ...
    async def test_double_mark_is_idempotent(self, db):
...
class TestMarketMoveNotAlerted:
    async def test_market_move_row_not_marked(self, db):
        """
...
class TestBlockedPickNotAlerted:
    async def test_blocked_pick_stays_false(self, db):
...
class TestGradingAlertedPicks:
    ...
    async def _setup_alerted_row(self, db, external_id: str, stat_type: str,
                                 line: float, recommendation: str = "OVER"):
        """Log a prop, mark it as alerted, return the row."""
        ...
        return await _fetch_opp(db, external_id, stat_type)

    async def test_grade_hit(self, db):
        """Telegram pick with actual > line → result=HIT."""
        row = await self._setup_alerted_row(db, "ext_h", "hits", line=0.5)
    ...
    async def test_grade_miss(self, db):
        """Telegram pick with actual < line → result=MISS."""
        row = await self._setup_alerted_row(db, "ext_m", "hits", line=0.5)
    ...
    async def test_grade_push(self, db):
        """Telegram pick with actual == line → result=PUSH."""
        row = await self._setup_alerted_row(db, "ext_p", "hits", line=1.0)
...
class TestOriginalLinePreserved:
    async def test_re_evaluation_does_not_overwrite_alert_sent(self, db):
        """
    ...
    async def test_alert_sent_at_preserved_after_upsert(self, db):
...
class TestTelegramOnlyPerformance:
    async def test_performance_excludes_non_alerted_props(self, db):
    ...
    async def test_performance_counts_by_result(self, db):
    ...
    async def test_hit_rate_calculation(self, db):
    ...
    async def test_zero_picks_returns_zero_totals(self, db):
    ...
    async def test_pass_recommendations_excluded(self, db):
...
class TestOverallGradingUnchanged:
    async def test_overall_grading_includes_all_evaluated_props(self, db):
        """
    ...
    async def test_grade_opportunity_works_on_non_alerted_row(self, db):
...
class TestSlipRelationshipPreserved:
    async def test_opportunity_id_still_links_to_prop_opportunity_log(self, db):
        """
...
class TestPendingBehavior:
    async def test_alerted_pick_starts_pending(self, db):
    ...
    async def test_pending_pick_not_in_graded_count(self, db):
    ...
    async def test_pending_pick_not_graded_by_missing_result_data(self, db):
...
class TestSchemaSanity:
    async def test_alert_sent_column_exists_in_model(self):
    ...
    async def test_mark_opportunity_alert_sent_method_exists(self):
        """Database has mark_opportunity_alert_sent method."""
        from database import Database
        assert hasattr(Database, "mark_opportunity_alert_sent")

    async def test_get_telegram_pick_performance_method_exists(self):
