...
class TestTierPerf:
    async def test_tier_emoji_s(self):
        assert TierPerf(tier="S", count=5).tier_emoji == "🔥"

    async def test_tier_emoji_a(self):
        assert TierPerf(tier="A", count=3).tier_emoji == "🟢"

    async def test_tier_emoji_b(self):
        assert TierPerf(tier="B", count=2).tier_emoji == "🟡"

    async def test_tier_emoji_pass(self):
        assert TierPerf(tier="PASS", count=0).tier_emoji == "⚪"

    async def test_tier_emoji_unknown(self):
...
class TestSportPerf:
    async def test_total_sums_all(self):
        sp = SportPerf(sport="MLB", ev_count=3, ud_count=5, pp_count=2)
        assert sp.total == 10

    async def test_total_zero_by_default(self):
        sp = SportPerf(sport="NFL")
        assert sp.total == 0

    async def test_avg_ev_optional(self):
...
class TestDashboardReport:
    ...
    async def test_total_all_alerts_sums_types(self):
        r = DashboardReport(
            total_ev_alerts=10,
    ...
    async def test_total_all_alerts_zero_default(self):
        r = DashboardReport()
    ...
    async def test_to_telegram_returns_string(self):
        r = DashboardReport()
        msg = r.to_telegram()
        assert isinstance(msg, str)
        assert len(msg) > 0

    async def test_to_telegram_has_header(self):
        r = DashboardReport()
        msg = r.to_telegram()
        assert "Performance Dashboard" in msg

    async def test_to_telegram_has_alerts_section(self):
        r = DashboardReport(total_ud_alerts=3, today_ud_alerts=1)
        msg = r.to_telegram()
        assert "Activity Summary" in msg
        assert "3" in msg

    async def test_to_telegram_ev_section_with_data(self):
    ...
    async def test_to_telegram_ev_win_rate(self):
    ...
    async def test_to_telegram_no_win_rate_awaiting(self):
        # The "awaiting" placeholder is now in the CLV section
        r = DashboardReport(total_clv_records=0)
        msg = r.to_telegram()
        assert "No CLV records yet" in msg

    async def test_to_telegram_clv_section_no_records(self):
        r = DashboardReport(total_clv_records=0)
        msg = r.to_telegram()
        assert "CLV" in msg
        assert "No CLV records yet" in msg

    async def test_to_telegram_clv_section_with_records(self):
        r = DashboardReport(
    ...
    async def test_to_telegram_pending_seeds(self):
        r = DashboardReport(clv_seeds_pending=5)
        msg = r.to_telegram()
        assert "5" in msg
        assert "seeds" in msg.lower() or "Pending" in msg

    async def test_to_telegram_ud_breakdown_shown(self):
        r = DashboardReport(
            total_ud_alerts=20,
    ...
    async def test_to_telegram_by_sport(self):
        r = DashboardReport(
            by_sport=[
    ...
    async def test_to_telegram_by_market(self):
        r = DashboardReport(
            by_market=[
    ...
    async def test_to_telegram_daily_trend(self):
        r = DashboardReport(
            daily_trend=[
    ...
    async def test_to_telegram_best_sport_shown(self):
        r = DashboardReport(best_sport="MLB")
        r.by_sport = [SportPerf(sport="MLB", ev_count=5)]
        msg = r.to_telegram()
        assert "MLB" in msg

    async def test_to_telegram_positive_clv_has_plus(self):
        r = DashboardReport(total_clv_records=5, avg_clv_pct=1.50)
        msg = r.to_telegram()
        assert "+1.50" in msg

    async def test_to_telegram_negative_clv_has_minus(self):
        r = DashboardReport(total_clv_records=5, avg_clv_pct=-0.80)
        msg = r.to_telegram()
        assert "-0.80" in msg

    async def test_to_telegram_no_html_unclosed_tags(self):
...
class TestDashboardEngineEmpty:
    ...
    async def test_gather_returns_report(self, empty_db):
        report = await DashboardEngine.gather(empty_db)
        assert isinstance(report, DashboardReport)

    async def test_gather_all_totals_zero(self, empty_db):
        report = await DashboardEngine.gather(empty_db)
    ...
    async def test_gather_no_best_worst(self, empty_db):
        report = await DashboardEngine.gather(empty_db)
        assert report.best_sport  is None
        assert report.worst_sport is None
        assert report.best_market is None

    async def test_to_telegram_safe_on_empty(self, empty_db):
        report = await DashboardEngine.gather(empty_db)
...
class TestDashboardEngineWithData:
    ...
    @pytest.fixture()
    async def db_with_ev(self, empty_db):
        ...
        for i, (sport, market, ev, result) in enumerate([
            ...
            records.append(r)
        await _save_all(empty_db, records)
        return empty_db

    async def test_ev_count_matches_saved(self, db_with_ev):
        report = await DashboardEngine.gather(db_with_ev)
        assert report.total_ev_alerts == 5

    async def test_avg_ev_pct_computed(self, db_with_ev):
        report = await DashboardEngine.gather(db_with_ev)
        # avg of 4.5, 3.2, 6.0, 2.1, 5.5 = 21.3/5 = 4.26
        assert report.avg_ev_pct is not None
        assert abs(report.avg_ev_pct - 4.26) < 0.1

    async def test_win_rate_from_resolved(self, db_with_ev):
        report = await DashboardEngine.gather(db_with_ev)
    ...
    async def test_by_sport_has_mlb_and_nba(self, db_with_ev):
        report = await DashboardEngine.gather(db_with_ev)
        sports = {sp.sport for sp in report.by_sport}
        assert "MLB" in sports
        assert "NBA" in sports

    async def test_by_market_has_player_prop(self, db_with_ev):
        report = await DashboardEngine.gather(db_with_ev)
        markets = {m.market for m in report.by_market}
        assert "Player Prop" in markets

    async def test_daily_trend_has_today(self, db_with_ev):
    ...
    async def test_to_telegram_includes_sport_data(self, db_with_ev):
        report = await DashboardEngine.gather(db_with_ev)
        msg = report.to_telegram()
