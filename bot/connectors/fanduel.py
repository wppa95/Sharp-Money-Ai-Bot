...
class FanDuelConnector(BaseConnector):
    ...
    async def _fetch_sport(self, sport: str, sport_key: str) -> list[MarketSnapshot]:
        ...
        fetches ALL bookmakers in one shared call (same entry as DraftKings);
        _normalize() filters for FanDuel data client-side, halving API usage.
        """
        try:
        ...
        try:
            ...
            logger.warning("FanDuel/OddsAPI error for %s: %s", sport, exc)
            return []
        except Exception as exc:  # noqa: BLE001
            logger.warning("FanDuel unexpected error (%s): %s", sport, exc)
            return []
